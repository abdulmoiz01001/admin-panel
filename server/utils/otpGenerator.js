...
async function  storeAccountVerificationOTP (userId, otp) {
  try {
    const otpHash = hashOTP(otp);
    const timestamp = Date.now();
    const user = await User.findById(userId)
    ...
    user.accountVerificationOTP = otpHash
    user.otpExpires = timestamp
    await user.save()
  } catch (error) {
    throw new Error('Error storing OTP: ' + error.message);
...
async function verifyOTP(userId, submittedOtp,res,purpose) {
   ...
   if(purpose === "accountVerification"){
    ...
    if (submittedOtpHash === storedOtpHash) {
      ...
      user.otpExpires = ''
      user.accountVerificated = true
      user.save()
      res.send('your account has verified successfully');
...
module.exports = {generateOTP,verifyOTP,storeOTP,storeAccountVerificationOTP}